...
| 2d   | medium | sonnet | none      | **Descoped during execution.** The original step called for a 60-sample bandwidth ring threaded into the eframe bug-report metadata, but bug-report capture is a GUI-only feature in ryll today — web mode has no trigger surface for it (deferred to `PLAN-web-ui-convergence.md`). Threading the ring into a struct nothing reads would be premature plumbing. Replaced with an `info!`-level log line in `inputs.rs` on accepted SetBitrate dispatches (rate-limited server-side via the same 10% band-crossing filter the JS uses) so operators can watch the loop from `ryll`'s stdout without `--verbose`. Revisit when web-mode bug reports land. Shipped in commit `2bd468c1`. |
