...
Phase 3 of [PLAN-stream-caps-and-flap.md](PLAN-stream-caps-and-flap.md).
...
- H.264 / VP8 / VP9 video codecs — that's phase 4 (multi-codec).
...
| 3H | — | — | — | **Per-platform smoke tests.** Operator-driven. On each of macOS, Windows, and Linux, run a fresh ryll session against a real spice-server with a workload that exercises MJPEG (drag a window for 5 seconds, scroll a video player, etc.). File a Display bug report and verify: (a) `mjpeg_decoder_backend` shows the expected platform-native value (not "jpeg-decoder"); (b) `mjpeg_decode_recent_mean_us` is well under the prior pure-Rust baseline (target: ≤30 ms at 2048×1152 on macOS Apple Silicon; ≤40 ms on Windows with WIC; ≤40 ms on Linux with mozjpeg, ≤20 ms with VA-API). Writeup goes into the master plan's *Bugs fixed during this work* if anything went wrong. |
...
decoder selection logic; phase 8 (docs) covers
