...
| 12G | low | haiku | none | **Docs catchup for 12E + 12F.** Document `--glz-dictionary-cap-mib` in `docs/configuration.md`. Add a "Glz dictionary pressure" subsection to `docs/troubleshooting.md` parallel to the existing "Display image cache pressure" section. Note the `image_cache_bytes` field-semantics change explicitly (it no longer includes GLZ). Cross-link from `docs/libvirt-spice-recommendations.md`. Verify `pre-commit run --all-files`. |
