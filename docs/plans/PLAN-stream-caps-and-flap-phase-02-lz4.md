...
Phase 2 of [PLAN-stream-caps-and-flap.md](PLAN-stream-caps-and-flap.md).
...
- Wiring an LZ4-vs-Zlib preference into a future
  `PREF_COMPRESSION` message — that lands in phase 4.
...
| 2C | — | — | — | **Manual smoke test.** After 2B lands, the operator runs a real ryll session against the test VM and confirms (a) the link handshake negotiates cap 5 (b) `channel-state.json::recent_decodes` shows at least one entry with `image_type: "Some(Lz4)"` or that none appear because the workload didn't need LZ4 (c) no `LZ4` warnings in the log. This step is operator-driven; no sub-agent. Writeup goes into the master plan's *Bugs fixed during this work* if anything went wrong. |
...
entry. Strictly that's phase 7's job; if step 2B's
