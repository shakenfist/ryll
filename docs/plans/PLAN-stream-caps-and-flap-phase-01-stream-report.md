...
- The "unsupported codec" special-case signal
  ...
  matters when we accept multi-codec streams (phase 5), so we
  fold it in there alongside H.264 decode failures.
- Adaptive UI surfacing of report cadence — `channel-state.json`
  is enough for now.
- Cross-validation against virt-viewer / spice-gtk. The phase 5
...
pub stream_reports_unsupported_signals_sent: u64,  // for phase 5 — initialised to 0
```
...
| 1G | low | haiku | none | **Snapshot test fields + aggregate counter.** Extend `DisplaySnapshot` with `stream_reports_sent_total: u64` and `stream_reports_unsupported_signals_sent: u64` (the latter initialised to 0 — written in phase 5). Wire both through `update_snapshot` in `display.rs`. Extend the `test_display_snapshot_serialises` test in `ryll/src/bugreport.rs` to assert these appear in the JSON output. Verify `make build && make test && make lint`. |
...
that's phase 9's job, but the implementing agent for step 1A
