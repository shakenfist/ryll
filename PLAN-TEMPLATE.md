...
Record each phase's merge commit in the Status column of that
table as the phase lands. The push-audit phase needs it: once
the phase branches are gone, the plan's accumulated diff is
not derivable from `develop` alone. See *Two ways this runbook
is invoked* in `PUSH-AUDIT.md`.
